...
test('visual vision service attaches two cached images without exposing local paths', async () => {
  const workspace = createTempWorkspace()
  const paths = {
  ...
  const service = createVisualVisionService({ paths, cacheDir: path.join(workspace, '.cache', 'visual', 'ai-review'), maxWidth: 100 })
  const result = await service.attachVisionInput({ visualAssets: { figmaRenderId: 'render-1', webScreenshotFileName: 'aaaaaaaaaaaaaaaaaaaaaaaa.png' }, visualEvidence: { media: {} } })

  assert.equal(result.meta.visionPrepared, true)
  assert.equal(result.payload.visionInput.images.figma.dataUrl.startsWith('data:image/jpeg;base64,'), true)
  assert.equal(result.payload.visionInput.images.web.dataUrl.startsWith('data:image/jpeg;base64,'), true)
