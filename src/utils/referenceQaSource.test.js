...
test('Reference URL QA table moves reason to details and keeps URL cells compact', () => {
  ...
  assert.equal(panelSource.includes('1차 메뉴'), true)
  assert.equal(panelSource.includes('2차 메뉴'), true)
  assert.equal(panelSource.includes('페이지/항목'), true)
  assert.equal(panelSource.includes('원본 행'), true)
  assert.equal(panelSource.includes('NavigationIntentDetails'), true)
  assert.equal(panelSource.includes("getSectionVisibility(rows, { maxVisible: 5, statusOrder: ['error', 'warn', 'info', 'ok'] })"), true)
